...
class intelligent_suggest:
    def __init__(self, word_list_fileName, word_index_fileName):
        ...
        if not os.path.isfile(word_list_fileName):
            raise IOError("Error Word list -%s-file does not exist" % word_list_fileName)
        if not os.path.isfile(word_index_fileName):
            raise IOError("Error Word list -%s-file does not exist" % word_index_fileName)
    ...
    def sanityCheck(self):
        if not isinstance(self.spellCheckObj, SpellCheck):
            raise Exception("Error there is something wrong with teh spell check object")
    ...
    def filter_results(self, query, candidates):
        ...
        for c, score in candidates.iteritems():
            ...
            if (query[0].lower() != word[0].lower()) or lDiff > self.lengthDiff:
                continue
            if word is None:
                continue
    ...
    def calcDistance(self, str1, str2):
        ...
        return Levenshtein.distance(str1, str2) / len(str2) * 100
    ...
    def process_word_index(self):
        ...
        for line in contents.values():
            if line[0] is None or line[1] is None or line[2] is None or line[3] is None or line[4] is None:
                ...
                self.addWordToIndex(line[1], {"weight": float(line[3]), "assoc": line[4]})
    ...
    def suggest(self, query):
        ...
        if self.spellCheckObj.inList(query) or query == "":
            ...
            if len(candidates) > 0:
                ...
                return self.filter_results(query, self.ktuple_candidate_search(query))
